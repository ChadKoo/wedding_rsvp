...
rsvpForm.addEventListener('submit', (e) => {
    ...
    .then(data => {
        ...
        // Add the correct data-id to the list item
        attendeeItem.setAttribute('data-id', data.id);
...
function cancelReservation(attendeeId) {
    ...
    fetch(`hhttp://localhost:3000/attendees/${attendeeId}`, {
        method: 'DELETE',
    })
    .then(response => {
        if (!response.ok) {
        ...
        if (attendeeItem) {
            ...
            console.error(`No attendee item found with ID ${attendeeId}`);
        }
    })
    .catch((error) => {
        console.error('Error:', error);
